(ns rpg.world
    (:require [reagent.core :as reagent :refer [atom]]
              ...
              [rpg.tools :refer [round timer atom-input]]
              [ajax.core :refer [GET POST DELETE]]
              [cljs.reader :refer [read-string]]))

(defn rgba [r g b a] (str "rgba(" r "," g "," b "," a ")"))

(defn gametable [data profile sheet]
    ...
    [:div.gametable
      (map (fn [data] [:div.card-state
                      {:style {:width (str (/ (- 100 (:padding @ux-view)) @x-view) "%")
                               :height (str (/ (- 100 (:padding @ux-view)) @y-view) "vh")
                               :left (str (* (:x data) (/ (- 100 (:padding @ux-view)) @x-view)) "%")
                               :top (str (* (:y data) (/ (- 100 (:padding @ux-view)) @y-view)) "vh")
                               :position "fixed"
                               :z-index (:z data)
